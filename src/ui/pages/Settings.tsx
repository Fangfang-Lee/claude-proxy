...
// 解析 Claude Code 配置为 env 对象
function parseClaudeConfig(content: string): Record<string, string> {
  try {
...
// 将 env 对象转换为完整的 Claude Code 配置 JSON
function envToClaudeConfig(env: Record<string, string>): string {
  const config = {
    env,
  }
...
export default function SettingsPage({ onRestart }: Props) {
  ...
  const loadClaudeConfig = async () => {
    ...
    if (result.success && result.content) {
      ...
      // 同步到表单
      const env = parseClaudeConfig(result.content)
  ...
  // 切换到 JSON 视图时，同步当前表单数据到 JSON
  const switchToJson = () => {
    const configJson = envToClaudeConfig(currentEnv)
    setClaudeConfig(configJson)
    setJsonError('')
    setViewMode('json')
  }
  ...
  const switchToForm = () => {
    try {
      const env = parseClaudeConfig(claudeConfig)
      const newAliases = { ...aliases } as Record<AliasKey, string>
      for (const k of MODEL_ALIAS_KEYS) {
        newAliases[k] = env[k] ?? ''
      }
  ...
  const handleJsonChange = (value: string) => {
    setClaudeConfig(value)
    setJsonError('')
    // 尝试解析并同步到表单
    try {
      const env = parseClaudeConfig(value)
      const newAliases = { ...aliases } as Record<AliasKey, string>
      for (const k of MODEL_ALIAS_KEYS) {
        newAliases[k] = env[k] ?? ''
      }
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="space-y-6">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          {viewMode === 'form' && (
            <>
              <p className="text-xs text-gray-400 mb-5">填写后写入 Claude Code 配置时生效，留空则不写入该项</p>
          ...
          {viewMode === 'json' && (
            <>
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                {configMsg && (
                  <span className={`text-sm ${configMsg.includes('失败') || configMsg.includes('不能为空') ? 'text-red-500' : 'text-green-600'}`}>
                    {configMsg}
                  </span>
                )}
              </div>
              <textarea
                ...
                placeholder='点击"加载配置"读取当前配置文件，或切换到表单视图填写'
              />
